"""You saw the first three functions in the binary_strings program from exercise 1. list_strings gives a list of all strings of length n, string_to_nat and nat_to_string convert back and forth between binary strings and natural numbers. These are really just used for testing purposes for this assignment"""
...
"""This next block of functions is setting us up to program in AON-Straightline. The is_binary functino checks that we are only computing on binary inputs/outputs (the only data type that AON-Straightlin can handle). The AND, OR, and NOT functions implement the corresponding boolean operators in Java. """
...
"""Now you're ready to start implementing programs in AON straightline for yourself. Implement the 3 functions (IMPL, PAL5, COMPARE4) described below. (This function will not be assessed for a grade, so you may skip it if you like)"""
...
"""Checks that your implementation of PAL5 is correct. It enumerates all inputs(note there are only finitely many, so we can be exhaustive), and checks the answer"""
for a in list_strings(5):
    if a == a[::-1]:
        assert(PAL5(a[0], a[1], a[2], a[3], a[4]) == '1')
    else:
        assert(PAL5(a[0], a[1], a[2], a[3], a[4]) == '0')
print('a nut for a jar of tuna')
